...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', 
             init_gain=0.02, gpu_ids=[], use_resize_conv = True):
    """Create a generator
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    """
    net = None
    norm_layer = get_norm_layer(norm_type="batch") # get_norm_layer(norm_type=norm)
...
class UnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, 
        ...
        unet_block = UnetSkipConnectionBlock(ngf * 8, ngf * 8, input_nc=None, submodule=None, 
                                             ...
                                             use_resize_conv = use_resize_conv)  # add the innermost layer
        ...
        self.model = UnetSkipConnectionBlock(output_nc, ngf, input_nc=input_nc, 
                                             ...
                                             use_resize_conv = use_resize_conv)  # add the outermost layer
...
class UnetSkipConnectionBlock(nn.Module):
    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        Parameters:
            ...
            norm_layer          -- normalization layer
            use_dropout (bool)  -- if use dropout layers.
            use_resize_conv (bool) -- whether to use resize-convolution to replace transpose convolution
        """
        ...
        if outermost:
            if use_resize_conv:
                upconv = nn.Sequential( nn.UpsamplingNearest2d(scale_factor = 2), 
    ...
    def forward(self, x):
        if self.outermost:
            ...
            res = torch.cat([x, self.model(x)], 1)
        return res
